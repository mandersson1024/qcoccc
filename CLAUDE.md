...
**Weighted ANY for personal interest selection:** When the user picks ANY for a personal interest, skills are weighted by occupation relevance using `random.choices`. Each occupation JSON may carry a `personal_interest_weights` map (skill or group name → weight multiplier). Keys use the group prefix for specializations (e.g. `"Art/Craft"` covers all Art/Craft variants) and the plain name for non-specializations (e.g. `"Listen"`). Skills absent from the map default to weight 1. The weights apply only to personal interest ANY resolution — occupation skill selection (including `{"choice": "any"}` entries) is unaffected and uses uniform random. 24 occupations currently have weights populated; the remaining occupations default to uniform random for personal interests.
